...
function decodeAutoconfigTokenV1(token) {
	if (!token.startsWith(AUTOCONFIG_TOKEN_PREFIX_V1)) throw new AutoConfigError(UNSUPPORTED_TOKEN_VERSION);
	const b64 = token.slice(8);
	let json;
	try {
...
var AutoConfig = class {
	...
	subscribe() {
		return new Endpoint(this.requestCtx).autoConfig.update(this.appId, this.endpointId, { endpoint: this.endpointIn });
	}
...
var AutoConfigConsumer = class {
	...
	subscribe() {
		return new Endpoint(this.requestCtx).autoConfig.update(this.appId, this.sinkId, { sink: {
			type: "poller",
			config: this.sinkIn
		} });
	}
	receive(consumerId, options) {
		return new MessagePollerv2(this.requestCtx).consumerPoll(this.appId, this.sinkId, consumerId, options);
	}
	commit(consumerId, offset, options) {
		return new MessagePollerv2(this.requestCtx).consumerCommit(this.appId, this.sinkId, consumerId, { offset }, options);
	}
};
//#endregion
//#region src/index.ts
...
//#endregion
export { ApiException, AppPortalCapability, AutoConfig, AutoConfigConsumer, AutoConfigError, BackgroundTaskStatus, BackgroundTaskType, BulkExpungeStatus, ConnectorKind, ConnectorProduct, HTTPValidationError, HttpErrorOut, MessageAttemptTriggerType, MessageStatus, MessageStatusText, Ordering, SinkStatus, SinkStatusIn, StatusCodeClass, Svix, ValidationError, Webhook, WebhookVerificationError, messageInRaw };
